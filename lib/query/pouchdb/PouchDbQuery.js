...
var PouchDbQuery = (function () {
    ...
    PouchDbQuery.prototype.extractSubQueries = function () {
        ...
        for (var propertyName in this.queryJson) {
            var childEntityName = entityRelationPropertyMap[propertyName];
            if (childEntityName) {
                var fragmentJson = this.queryJson[propertyName];
                var childQuery = new PouchDbQuery(childEntityName, this.entitiesRelationPropertyMap, this.entitiesPropertyTypeMap, fragmentJson);
    ...
    PouchDbQuery.prototype.extractJoinField = function (fieldName, fragment) {
        ...
        var _loop_1 = function(operator) {
            ...
            if (joinToEntity && joinToField) {
                ...
                joinField = new JoinFieldNode(subFragment[PHQuery_1.PH_JOIN_TO_ENTITY], subFragment[PHQuery_1.PH_JOIN_TO_FIELD], operator);
