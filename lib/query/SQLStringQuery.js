"use strict";
var PHSQLQuery_1 = require("./sql/PHSQLQuery");
...
var SQLAdaptor_1 = require("./sql/adaptor/SQLAdaptor");
/**
 * Created by Papa on 8/20/2016.
 */
(function (SQLDialect) {
    SQLDialect[SQLDialect["MY_SQL"] = 0] = "MY_SQL";
...
var SQLQuery = (function () {
    function SQLQuery(phJsonQuery, qEntity, qEntityMap, entitiesRelationPropertyMap, entitiesPropertyTypeMap, dialect) {
    ...
    SQLQuery.prototype.toSQL = function () {
        ...
        var joinQEntityMap = {};
        var joinAliasMap = {};
        var fromFragment = this.getFromFragment(joinQEntityMap, joinAliasMap);
        var selectFragment = this.getSelectFragment(entityName, null, this.phJsonQuery.select, joinAliasMap);
        var whereFragment = this.getWHEREFragment(this.phJsonQuery.where, 0, joinQEntityMap);
        return "SELECT\n" + selectFragment + "\nFROM\n" + fromFragment + "\nWHERE\n" + whereFragment;
    };
    SQLQuery.prototype.getSelectFragment = function (entityName, existingSelectFragment, selectClauseFragment, joinAliasMap) {
        ...
        for (var propertyName in selectClauseFragment) {
            if (entityPropertyTypeMap[propertyName]) {
                ...
                selectFragment += this.getSelectFragment(entityRelationMap[propertyName].entityName, existingSelectFragment + selectFragment, selectClauseFragment[propertyName], joinAliasMap);
            }
            else {
    ...
    SQLQuery.prototype.getFromFragment = function (joinQEntityMap, joinAliasMap) {
    ...
    SQLQuery.prototype.getWHEREFragment = function (operation, nestingIndex, joinQEntityMap) {
        var _this = this;
        var whereFragment = '';
        var nestingPrefix = '';
    ...
    SQLQuery.prototype.getComparibleOperatorAndValueFragment = function (fieldOperation, value, alias, propertyName, typeCheckFunction, typeName, conversionFunction) {
    ...
    SQLQuery.prototype.getCommonOperatorAndValueFragment = function (fieldOperation, value, alias, propertyName, typeCheckFunction, typeName, conversionFunction) {
        var _this = this;
        var sqlOperator;
        switch (fieldOperation) {
            case '$eq':
    ...
    SQLQuery.prototype.booleanTypeCheck = function (valueToCheck) {
        return typeof valueToCheck === 'boolean';
    };
    SQLQuery.prototype.dateTypeCheck = function (valueToCheck) {
    ...
    SQLQuery.prototype.numberTypeCheck = function (valueToCheck) {
        return typeof valueToCheck === 'number';
    };
    SQLQuery.prototype.stringTypeCheck = function (valueToCheck) {
        return typeof valueToCheck === 'string';
    };
    SQLQuery.prototype.throwValueOnOperationError = function (valueType, operation, alias, propertyName) {
        throw "Expecting a string value for $eq (=) operation on '" + alias + "." + propertyName + "' used in the WHERE clause.";
    };
    SQLQuery.prototype.sanitizeStringValue = function (value) {
        // FIXME: sanitize the string to prevent SQL Injection attacks.
        return "'" + value + "'";
    };
    SQLQuery.prototype.warn = function (warning) {
        console.log(warning);
    };
    SQLQuery.prototype.parseResults = function (selectClauseFragment, results) {
        var entityName = this.qEntity.__entityName__;
        var numResultColumns = 0;
    };
    SQLQuery.prototype.parseQueryResults = function (entityName, selectClauseFragment, results, numResultColumns) {
        var _this = this;
        var parsedResults = [];
        ...
        return results.map(function (result) {
            return _this.parseQueryResult(entityName, {}, selectClauseFragment, result, numResultColumns, 0);
        });
    };
    SQLQuery.prototype.parseQueryResult = function (entityName, resultObject, selectClauseFragment, result, numResultColumns, nextFieldIndex) {
        // Return blanks, primitives and Dates directly
        if (!result || !(result instanceof Object) || result instanceof Date) {
            return result;
        }
        var qEntity = this.qEntityMap[entityName];
        var entityMetadata = qEntity.__entityConstructor__;
