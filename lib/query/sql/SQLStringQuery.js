...
var SQLStringQuery = (function () {
    ...
    SQLStringQuery.prototype.getSelectFragment = function (entityName, selectFragment, selectClauseFragment, joinAliasMap, columnAliasMap, entityDefaultsMap) {
        ...
        if (numProperties === 0) {
            ...
            for (var propertyName in entityRelationMap) {
                selectClauseFragment[propertyName] = {};
            }
        }
        for (var propertyName in selectClauseFragment) {
            ...
            if (entityPropertyTypeMap[propertyName]) {
                var columnSelect = this.getColumnSelectFragment(entityName, propertyName, tableAlias, columnMap, null, columnAliasMap, selectFragment);
                ...
                if (subSelectClauseFragment == null) {
                    if (entityMetadata.manyToOneMap[propertyName]) {
                        var columnSelect = this.getColumnSelectFragment(entityName, propertyName, tableAlias, null, joinColumnMap, columnAliasMap, selectFragment);
                        selectFragment += columnSelect;
                        continue;
                    }
                    else {
    ...
    SQLStringQuery.prototype.getColumnSelectFragment = function (entityName, propertyName, tableAlias, columnMap, joinColumnMap, columnAliasMap, existingSelectFragment) {
        var columnName;
    ...
    SQLStringQuery.prototype.getFromFragment = function (joinQEntityMap, joinAliasMap) {
        ...
        for (var i = 1; i < joinRelations.length; i++) {
            ...
            if (rightEntityMetadata.manyToOneMap[joinRelation.relationPropertyName]) {
                var rightEntityJoinColumnMetadata = rightEntityMetadata.joinColumnMap[joinRelation.relationPropertyName];
                ...
                var leftEntityJoinColumnMetadata = rightEntityMetadata.joinColumnMap[mappedByLeftEntityProperty];
