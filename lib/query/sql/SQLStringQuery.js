...
(function (QueryResultType) {
    ...
    QueryResultType[QueryResultType["FLAT"] = 1] = "FLAT";
    // A flat array of values, returned by a on object select
    QueryResultType[QueryResultType["FLATTENED"] = 2] = "FLATTENED";
    // Ordered query result, with objects grouped hierarchically by entity
    QueryResultType[QueryResultType["HIERARCHICAL"] = 3] = "HIERARCHICAL";
    // A mapped array of values, returned by a regular join
    QueryResultType[QueryResultType["MAPPED"] = 4] = "MAPPED";
    // Plain query result, with no forced ordering or grouping
    QueryResultType[QueryResultType["PLAIN"] = 5] = "PLAIN";
    // Raw result, returned by a SQL string query
    QueryResultType[QueryResultType["RAW"] = 6] = "RAW";
...
var SQLStringQuery = (function (_super) {
    ...
    function SQLStringQuery(phJsonQuery, rootQEntity, qEntityMapByName, entitiesRelationPropertyMap, entitiesPropertyTypeMap, dialect, queryResultType) {
        _super.call(this, rootQEntity, qEntityMapByName, entitiesRelationPropertyMap, entitiesPropertyTypeMap, dialect);
    ...
    SQLStringQuery.prototype.getFROMFragment = function (parentTree, currentTree, embedParameters, parameters) {
        ...
        if (!parentTree) {
            ...
            switch (currentRelation.joinType) {
                case PHSQLQuery_1.JoinType.FULL_JOIN:
                ...
                case PHSQLQuery_1.JoinType.LEFT_JOIN:
                    ...
                    break;
                case PHSQLQuery_1.JoinType.RIGHT_JOIN:
                    if (this.queryResultType !== QueryResultType.FLAT) {
                        throw "Full Joins only allowed in flat queries";
                    }
