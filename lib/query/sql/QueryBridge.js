...
var QueryBridge = (function () {
    ...
    QueryBridge.prototype.bufferManyToOneStub = function (currentJoinNode, qEntity, entityMetadata, resultObject, propertyName, relationQEntity, relationEntityMetadata, relatedEntityId) {
        if (!this.performBridging) {
            if (Relation_1.QRelation.isStub(resultObject[propertyName])) {
                this.lastObjectTracker.addManyToOneReference(Relation_1.QRelation.getAlias(currentJoinNode.jsonRelation), resultObject, propertyName, relationEntityMetadata.idProperty);
            }
            return;
        }
        var otmEntityField;
