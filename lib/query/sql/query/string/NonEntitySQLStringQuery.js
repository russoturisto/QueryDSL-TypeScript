...
var NonEntitySQLStringQuery = (function (_super) {
    ...
    NonEntitySQLStringQuery.prototype.toSQL = function () {
        var jsonQuery = this.phJsonQuery;
        var joinNodeMap = {};
        this.joinTrees = this.buildFromJoinTree(jsonQuery.from, joinNodeMap);
        var selectFragment = this.getSELECTFragment(null, jsonQuery.select);
        var fromFragment = this.getFROMFragments(this.joinTrees);
        var whereFragment = '';
        if (jsonQuery.where) {
    ...
    NonEntitySQLStringQuery.prototype.getFieldSelectFragment = function (value, clauseType, nestedObjectCallBack, selectSqlFragment) {
        var columnSelectSqlFragment = this.getFieldValue(value, clauseType, 
        ...
        switch (fieldJson.objectType) {
            case Appliable_1.JSONClauseObjectType.FIELD_FUNCTION:
                switch (fieldJson.dataType) {
                    case Appliable_1.SQLDataType.BOOLEAN:
    ...
    NonEntitySQLStringQuery.prototype.getFROMFragments = function (joinTrees) {
        var _this = this;
        return joinTrees.map(function (joinTree) { return _this.getFROMFragment(null, joinTree); }).join('\n');
    };
    NonEntitySQLStringQuery.prototype.getFROMFragment = function (parentTree, currentTree) {
        var fromFragment = '\t';
    ...
    NonEntitySQLStringQuery.prototype.getGroupByFragment = function (groupBy) {
        var _this = this;
        return groupBy.map(function (groupByField) {
            _this.validator.validateAliasedFieldAccess(groupByField.fieldAlias);
            return "" + groupByField.fieldAlias;
        }).join(', ');
    };
    NonEntitySQLStringQuery.prototype.getOrderByFragment = function (orderBy) {
        var _this = this;
