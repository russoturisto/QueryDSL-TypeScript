...
var QField = (function () {
    ...
    QField.prototype.toJSON = function (columnAliases) {
        var alias;
        if (columnAliases) {
        ...
        if (this.__fieldSubQuery__) {
            var subSelectQuery = new PHFieldSQLQuery_1.PHFieldSQLQuery(this.__fieldSubQuery__).toJSON();
            jsonField.fieldSubQuery = subSelectQuery;
        }
        return jsonField;
    };
    QField.prototype.appliedFunctionsToJson = function (appliedFunctions) {
        var _this = this;
        if (!appliedFunctions) {
            return appliedFunctions;
        }
        return appliedFunctions.map(function (appliedFunction) {
            return _this.functionCallToJson(appliedFunction);
        });
    };
    QField.prototype.functionCallToJson = function (functionCall) {
        var _this = this;
        var parameters;
        if (functionCall.parameters) {
            parameters = functionCall.parameters.map(function (parameter) {
                return _this.valueToJSON(parameter);
            });
        }
        return {
    ...
    QField.prototype.valueToJSON = function (value) {
        if (!value) {
            return value;
        }
        switch (typeof value) {
    ...
    QField.prototype.operableFunctionToJson = function (type, value, columnAliases) {
        var alias;
        if (columnAliases) {
            alias = columnAliases.getNextAlias(this);
        }
