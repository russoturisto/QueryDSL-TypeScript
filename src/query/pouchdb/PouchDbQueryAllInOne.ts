 ...
 * same JSON tree.
 */
export class PouchDbQueryAllInOne {

	childSelectJson:{[propertyName:string]:any} = {};
	childQueries:{[propertyName:string]:PouchDbQueryAllInOne} = {};
	...
	extractSubQueries():void {
		...
		for (let propertyName in this.queryJson) {
			...
			if (relationRecord) {
				...
				let childQuery = new PouchDbQueryAllInOne(relationRecord.entityName, this.entitiesRelationPropertyMap, this.entitiesPropertyTypeMap, fragmentJson);
